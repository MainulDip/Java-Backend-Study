...
Java `record` is introduced in 14 to get rid of all the boilerplate code used in POJO for defining Data type like database results, query results, or information from a service. Records are immutable data classes that require only the type and name of fields. Records can optionally have member method and static properties.

With `records`, equals, hashCode, and toString methods, as well as the private, final fields and public constructor, are generated by the Java compiler. Custom constructor are also allowed
```java
...
* Creating a Mutable Type with @Data and @AllArgsConstructor annotations
...
Records don't support builder pattern, only constructor pattern. LomBok support Builder pattern

LomBok support inheritance with classes annotated with `@Value`, but they are final (cannot be a base class of another class). `record` doesn't support inheritance.
